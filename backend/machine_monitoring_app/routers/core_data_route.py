...
from machine_monitoring_app.models.response_models import CurrentData, FullTimelineData, StatusSummaryResponseData, \
    ...
    DisconnectionHistoryResponse, ParameterComparisonOutput_mongodb, CycleTimeFactoryLayout, CycleTimeMachineDetails, \
    CycleTimeMachineList
...
@ROUTER.get("/cycle-time/factory-layout", response_model=CycleTimeFactoryLayout)
async def get_cycle_time_layout(
    fromTime: Optional[str] = None,
    toTime: Optional[str] = None
):
    """
    GET CYCLE TIME FACTORY LAYOUT
    ===============================

    This API is used to query the cycle time status of all machines in the factory within a time range.

    Query Parameters:
    - fromTime: Start time as ISO format date-time string (optional, default: 1 hour ago in IST)
                Example: "2026-05-21 14:45:04" or "2026-05-21T14:45:04"
    - toTime: End time as ISO format date-time string (optional, default: current time in IST)
              Example: "2026-05-21 15:45:04" or "2026-05-21T15:45:04"
    """
    start_time = time.time()
    try:
        ...
        # Parse ISO format date-time strings
        if fromTime:
            from_time = datetime.fromisoformat(fromTime.replace(' ', 'T'))
            if from_time.tzinfo is None:
                from_time = ist_tz.localize(from_time)
        else:
            from_time = None

        if toTime:
            to_time = datetime.fromisoformat(toTime.replace(' ', 'T'))
            if to_time.tzinfo is None:
                to_time = ist_tz.localize(to_time)
        else:
            to_time = None

        response_data = get_cycle_time_factory_layout(from_time=from_time, to_time=to_time)
        end_time = time.time() - start_time
        LOGGER.info(f"Total Time Taken For this endpoint: {(round((end_time * 1000), 2))} ms")
        return response_data
    except Exception as error:
        LOGGER.error(f"Error fetching cycle time factory layout: {error}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve cycle time data: {str(error)}")


@ROUTER.get("/cycle-time/machine/{machineName}", response_model=CycleTimeMachineDetails)
async def get_cycle_time_machine(
    machineName: str,
    fromTime: Optional[str] = None,
    toTime: Optional[str] = None
):
    """
    GET CYCLE TIME MACHINE DETAILS
    ===============================

    This API is used to query the cycle time details for a specific machine within a time range.
    ...
    try:
        ...
        response_data = get_cycle_time_machine_details(machineName, from_time=from_time, to_time=to_time)
        end_time = time.time() - start_time
        LOGGER.info(f"Total Time Taken For this endpoint: {(round((end_time * 1000), 2))} ms")
        return response_data
        ...
        LOGGER.error(f"Error fetching cycle time machine details: {error}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve cycle time data: {str(error)}")


@ROUTER.get("/cycle-time/machines", response_model=CycleTimeMachineList)
async def get_cycle_time_machines():
    """
    GET ALL MACHINES FOR CYCLE TIME
    ===============================

    This API is used to query the list of all available machines for cycle time parameter.
    """
    start_time = time.time()
    try:
        machines = get_all_machines_for_cycle_time()
        response_data = {"machines": machines}
        end_time = time.time() - start_time
        LOGGER.info(f"Total Time Taken For this endpoint: {(round((end_time * 1000), 2))} ms")
        return response_data
    except Exception as error:
        LOGGER.error(f"Error fetching cycle time machines: {error}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve machines: {str(error)}")


@ROUTER.put("/cycle-time/limits/{machineName}")
async def update_cycle_time_limits(
    machineName: str,
    warning_limit: float = Query(...),
    critical_limit: float = Query(...)
):
    """
    UPDATE CYCLE TIME LIMITS FOR A MACHINE
    ======================================

    This API is used to update the warning and critical limits for cycle time for a specific machine.

    Path Parameters:
    - machineName: Name of the machine

    Query Parameters:
    - warning_limit: Warning limit value in seconds
    - critical_limit: Critical limit value in seconds
    """
    start_time = time.time()
    try:
        from machine_monitoring_app.database.crud_operations import update_cycle_time_limits
        result = update_cycle_time_limits(machineName, warning_limit, critical_limit)
        end_time = time.time() - start_time
        LOGGER.info(f"Total Time Taken For this endpoint: {(round((end_time * 1000), 2))} ms")
        return result
    except Exception as error:
