...
This script migrates only the user data schema (tiei_sample_5) from local to server.
...
def migrate_user_schema():
    """Migrate only the tiei_sample_5 schema (user data)."""
    print("\n" + "=" * 80)
    ...
    try:
        ...
        try:
            with server_conn.cursor() as cur:
                cur.execute(sql.SQL("CREATE SCHEMA IF NOT EXISTS {};").format(
                    sql.Identifier("tiei_sample_5")
                ))
            print("  Schema tiei_sample_5 created/verified")
        except Exception as e:
            print(f"  Warning creating schema: {e}")
            # Try alternative approach
            try:
                with server_conn.cursor() as cur:
                    cur.execute("CREATE SCHEMA IF NOT EXISTS tiei_sample_5;")
                print("  Schema tiei_sample_5 created/verified (alternative)")
            except Exception as e2:
                print(f"  Error creating schema: {e2}")
        
        # Get tables from local
        print("\n[Step 3/4] Creating tables...")
        tables = get_tables(local_conn, "tiei_sample_5")
        print(f"  Found {len(tables)} tables in tiei_sample_5")
        ...
        for table in tables_to_migrate:
            ...
            try:
                create_sql = generate_simple_create_table_sql(local_conn, "tiei_sample_5", table)
                with server_conn.cursor() as cur:
        ...
        for table in tables_to_migrate:
            ...
            migrate_table_data(local_conn, server_conn, "tiei_sample_5", table)
        
        # Verification
        print("\n[Verification]")
        local_tables = get_tables(local_conn, "tiei_sample_5")
        server_tables = get_tables(server_conn, "tiei_sample_5")
        ...
        for table in tables_to_migrate:
            local_count = get_row_count(local_conn, "tiei_sample_5", table)
            server_count = get_row_count(server_conn, "tiei_sample_5", table)
...
def main():
    ...
    print("This will migrate user data (tiei_sample_5 schema) from local to server")
    print("=" * 80)
