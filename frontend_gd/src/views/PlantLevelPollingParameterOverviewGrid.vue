...
const handleMachineClick = (machineName) => {
  if (isCycleTimeSelected.value) {
    ...
    window.location.href = `${window.location.origin}${import.meta.env.BASE_URL}#/machine-level-sampling?machine=${encodeURIComponent(machineName)}&parameter=CYCLE_TIME`;
  }
};
