...
from machine_monitoring_app.database.pony_models import Machine, ParameterGroup, MachineParameter, \
    ...
    UserAccessLog, CycleTime, CycleTimeLimits, PressureMonitoringMachine, PressureLogFile, Unit, schema_name
...
def get_cycle_time_factory_layout(from_time: datetime = None, to_time: datetime = None):
    """
    Retrieves cycle time data for all machines in factory layout format within a time range.

    Args:
        from_time: Start of time range (default: 1 hour ago in IST)
        to_time: End of time range (default: current time in IST)

    Returns:
        dict: Factory layout data with cycle time information for all machines
    """
    ...
    # Set default time range if not provided (using IST)
    if to_time is None:
        to_time = datetime.now(ist_tz)
    ...
    for location, location_machines in groupby(machines, key=lambda m: m.location):
        ...
        if line_data["count"]["CRITICAL"] > 0:
            line_data["line_state"] = "CRITICAL"
        elif line_data["count"]["WARNING"] > 0:
            line_data["line_state"] = "WARNING"

        # Only add line if it has machines with data
        if line_data["machines"]:
...
@db_session(optimistic=False)
def get_cycle_time_machine_details(machine_name: str, from_time: datetime = None, to_time: datetime = None):
    """
    Retrieves cycle time details for a specific machine within a time range.
    ...
    Returns:
        dict: Machine cycle time details including all data points within the time range
    """
    from datetime import datetime, timedelta
    import pytz
    ...
    cycle_time_list = list(cycle_time_data)

    # Convert to list of dictionaries
    cycle_time_records = []
    for ct in cycle_time_list:
        cycle_time_records.append({
            "time": ct.time.isoformat(),
            "cycle_time": ct.cycle_time
        })

    # Get the latest cycle time for state determination
    latest_cycle_time = cycle_time_list[-1] if cycle_time_list else None
    cycle_time_value = latest_cycle_time.cycle_time if latest_cycle_time else 0
    last_update = latest_cycle_time.time if latest_cycle_time else None

    # Determine state based on ALL values in the time range
    ...
    result = {
        ...
        "warning_limit": warning_limit,
        "critical_limit": critical_limit,
        "cycle_time_data": cycle_time_records
    }

    # Add message if no data found
    if not cycle_time_list:
        result["message"] = f"No cycle time data found for machine '{machine_name}' in the selected time range"

    return result


@db_session(optimistic=False)
def get_all_machines_for_cycle_time():
    """
    Retrieves all machines for cycle time parameter selection.
...
@db_session(optimistic=False)
def update_cycle_time_limits(machine_name: str, warning_limit: float, critical_limit: float):
    """
    Updates the warning and critical limits for cycle time for a specific machine.
    
    Args:
        machine_name (str): Name of the machine
        warning_limit (float): Warning limit value in seconds
        critical_limit (float): Critical limit value in seconds
